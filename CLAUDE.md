...
4. **동작 증명 — 가짜 아닌 real 왕복(DoD ④).** fake 백엔드/데모는 feel용 스모크일 뿐 *증명 아님*. 유형별 real:
   - **web** → `pnpm demo` 실브라우저(DPR·좌표·렌더 버그) + 필요시 실 스크린리더/HITL. (캔버스 버퍼=CSS×DPR, geometry 는 화면 박스 역산 `rect.h/ROWS`.) **a11y/UI 동작은 HITL 대신 `pnpm test:e2e`(Playwright 헤드리스, `webServer`가 `pnpm demo` 자동기동 → 실 wasm+컨트롤러 왕복)로 자동 증명** — headless가 못 하는 오디오/실 SR은 *SR-consumed 대리*로 단언(announce=aria-live `textContent`, signal=콘솔 로그; **억제 증명=SR off 후 그 둘이 *안* 나옴**). 라이브 눈확인은 Playwright MCP(`browser_navigate/snapshot/click/console`, 클릭 후 ref 무효→`button:has-text()` 셀렉터). 최초 1회 `pnpm exec playwright install chromium`. (#172 확립)
...
6. **게이트 & PR/머지.** 크레이트별 게이트 *전부*(사각 포함):
   ...
   - **web** → `pnpm typecheck` + 전체 vitest + `pnpm demo` (+ a11y/UI 관찰가능 변경이면 `pnpm test:e2e`). e2e 는 CI 미배선(로컬 게이트) — 브라우저 설치 필요.
