...
- **엔진 = `justerm-core`** (이 repo 의 코어 크레이트, 파싱+상태) / **렌더러 = `beamterm`** (그리드를
  WebGL2 로 그림, 별도) — 단 자체 렌더러 `justerm-renderer` 로 **전환 중**(ADR-0018, Epic #258; beamterm 은
  스위치 #273 까지 현역, 문서 플립은 #274). → `-term` 패밀리. `justerm` 은 *패밀리 umbrella* 이름이다(코어
...
- **상세 계약(구현 시 참조)**: **`docs/architecture.md`** — 셀·damage·뷰포트/스크롤·cadence·
  ...
  beamterm→0018 justerm-renderer). 큰 그림·빌드플랜은 GitHub **Epic #1**(엔진, closed) + 슬라이스
...
**우회 금지 (근본 층에서 고쳐라).** 구현 중 *다른 층(주로 core)의 결함/불완전*을 발견하면 — 소비처에서
*덮지(compensate) 마라*. 소비처 우회는 ① upstream 버그를 *가려* 고칠 압력을 없애고, ② 위 경계(예: width 는
core 책임)를 침범하며, ③ 같은 지식이 두 층에 중복돼 divergence 씨앗이 된다("테스트 통과시키려 소비처 코드로
돌아가는" 것 자체가 신호). **우회하고 싶은 충동이 들면 = 멈추고 먼저 사용자에게 온다 — *무슨 상황인지 설명*하고
("core 가 X 를 이렇게 넘겨서 소비처가 Y 해야 통과, 근본은 core 문제") "이거 조사/근본수정 해야 할 것 같다"고
*물어본다*.** 혼자 우회하지도, 혼자 이슈만 파고 넘어가지도 말고 판단을 사용자와 공유. 방향을 정한 뒤 (a) 근본
층에서 고치거나(정책 판단이면 grill/surface 후), (b) 당장 못 고치면 갭을 **보이게** 둔 채 이슈로 tracked —
테스트도 실동작을 정직히 단언(가짜 입력으로 통과시키지 말 것). 단
소비처가 *본래 자기 책임*인 일(색해석 등)은 우회가 아니다 — 경계는 "누구 책임이냐"로 가른다. 실증: #297/#300
(core VS16 width 갭을 렌더러 FE0F 감지로 우회 → 제지, 근본 수정은 #301 로 tracked).
...
4. **동작 증명 — 가짜 아닌 real 왕복(DoD ④).** fake 백엔드/데모는 feel용 스모크일 뿐 *증명 아님*. 유형별 real:
   - **web** → `pnpm demo` 실브라우저(DPR·좌표·렌더 버그) + 필요시 실 스크린리더/HITL. (캔버스 버퍼=CSS×DPR, geometry 는 화면 박스 역산 `rect.h/ROWS`.) **a11y/UI 동작은 HITL 대신 `pnpm test:e2e`(Playwright 헤드리스, `webServer`가 `pnpm demo` 자동기동 → 실 wasm+컨트롤러 왕복)로 자동 증명** — headless가 못 하는 오디오/실 SR은 *SR-consumed 대리*로 단언(announce=aria-live `textContent`, signal=콘솔 로그; **억제 증명=SR off 후 그 둘이 *안* 나옴**). **단 headless e2e 는 SR-consumed 대리(announce·signal)만 증명 — focus·scroll·reveal 같은 *시각/DOM 부수효과*는 안 짚는다. `e2e 그린 ≠ 시각효과 검증*.** web 수정에 그런 효과가 있으면 flow 에 Playwright 검증을 *반드시 포함*: (a) e2e 에서 DOM 상태를 직접 단언(`document.activeElement` 라인 인덱스·`scrollTop`) 하거나, (b) `pnpm demo` 를 백그라운드 기동 후 Playwright MCP 로 라이브 드라이브해 `browser_evaluate` 로 `activeElement`/scroll 을 읽어 확인(회귀방지는 결국 (a) 로 락인). 실증 #166: reveal focus 를 headless e2e 가 안 짚어(announce/signal 만 통과) 라이브 MCP 드라이브로만 확인됨 → 그 갭을 (a) 로 e2e 에 굳혀야 완결. 라이브 눈확인 = Playwright MCP(`browser_navigate/snapshot/click/console/evaluate`, 클릭 후 ref 무효→`button:has-text()` 셀렉터). 최초 1회 `pnpm exec playwright install chromium`. (#172 확립, #166 보강)
...
6. **게이트 & PR/머지.** 크레이트별 게이트 *전부*(사각 포함):
   ...
   - **web** → `pnpm typecheck` + 전체 vitest + `pnpm build` + `pnpm demo` (+ a11y/UI 관찰가능 변경이면 `pnpm test:e2e`; **focus·scroll·reveal 같은 시각/DOM 부수효과면 e2e 가 그 DOM 상태(`document.activeElement`·`scrollTop`)까지 단언** — announce/signal 만 보고 넘기면 미검증 갭, Step 4 참조). **#341 부터 CI 배선됨**(`web`, `web-e2e` 잡) — 로컬 e2e 는 브라우저 설치 필요.
     - `pnpm typecheck` 는 **tsconfig 3개**를 돌린다(#343): `tsconfig.json`(=`src`, 브라우저 — `types: []` 라 `process`·`Buffer` 는 *컴파일 에러*), `tsconfig.test.json`(`test`+`demo`+`e2e`, node 타입 허용), `tsconfig.node.json`(`*.config.ts`, DOM 없음). 셋 중 하나만 돌리면 조용히 커버리지가 샌다 — `tsc -p <각각> --listFiles` 로 추적 TS 파일 전부가 덮이는지 확인할 것(#341 이전엔 `e2e/` 가 통째로 타입검사 밖이었다).
     - `pnpm build`(tsup)는 **typecheck 이 놓친 타입에러를 잡지 못한다** — 변이 6종(emit-only TS4023·external 타입 DTS 롤업·bare type 재수출·`export *` 충돌·엔트리 부재)에서 전부 `tsc --noEmit` 이 *먼저* 빨개졌다(#344). 산출물 경로(엔트리 해석·번들·`dist/index.d.ts` 방출)만 지킨다. **여기 초록 = 공개 타입 건전, 아님.**
