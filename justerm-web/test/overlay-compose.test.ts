...
function op(x: number, y: number, bg = 0, fg = 0xffffff, blendHighlight = false, fgUndimmed = fg, dim = false): DrawOp {
  return { x, y, symbol: "a", fg, bg, bold: false, italic: false, underline: false, strikethrough: false, blendHighlight, fgUndimmed, dim, excludeFromContrast: false, inverseDefaultBg: false };
}
