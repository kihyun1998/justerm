...
- **header** — magic, version, flags, `cols`/`rows`, cursor (`cursor_row`/`cursor_col` u16 +
  ...
  (`marker_count` u32 — v16, #490: how many markers are live in the active buffer, so a consumer that
  wired the pull but not the create/dispose events notices the drift instead of painting decorations on
  lines it no longer owns), kind (`Full` | `Partial`).
...
- **The preceding grapheme is retained across bytes, and `REP` is its reader (#825, #865, #869).**
  `REP` (CSI Ps b) repeats what was last printed, so *"what was last printed"* becomes state the
  engine has to model. It briefly had a second reader and no longer does, which is worth one
  sentence because the round trip is the instructive part: #865 found that with autowrap off a print
  filling the last column pinned the cursor while the deferred-wrap flag stayed clear, so the
  combining-mark attach point read this state instead of the cursor — and #869 found that the flag
  was only mute because its *arm* folded the mode in, fixed that, and gave the reader back. **A
  second consumer of a piece of hidden state can be the symptom of a first one being wrong.** What is held is a **position, not a character**: `(row, col)` of the cell the last
...
- **Tab stops are explicit per-column state, not a fixed modulo.** A bool-per-column set: HTS
  ...
  is state the *application* wrote and `ESC c` is what resets the terminal. **CHT (CSI Ps I) is HT
  repeated `Ps` times**, stopping at the first repeat that does not move, so it inherits HT's
  right-edge rule (a parked cursor with no stop to reach stays parked) (#898). [#8]
...
- **Consumer events are pull-drained, and OSC 8 is not one of them.** Title (OSC 0/2, and since #823
  ...
  an OSC-terminating BEL is not double-counted as a bell. [#12] **"ST" here means the 7-bit
  `ESC \` only** — the 8-bit C1 `ST` (`0x9C`, and `C2 9C` for it) is not a terminator, because the
  stream is UTF-8 and a byte in `0x80..=0x9F` is ill-formed input rather than a control (#847).
  That is a contract: an OSC payload carries 8-bit text, so honouring `0x9C` byte-wise would
  truncate a title mid-character. **Which one arrived is retained and
...
- **An OSC 8 hyperlink is ambient pen-like state stamped onto cells — not an event, and not closed by
  ...
  **Two opens of an identical URI stay two links; an `id=` the application declared groups them.**
  ...
  (#650, matching xterm's deliberately special-cased split-on-first-`;`) — up to 13 of them, past
  which vte has already dropped the tail and the shorter URI is not marked (#840). It is never decoded: a
