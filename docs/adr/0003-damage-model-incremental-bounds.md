...
- **Aligned with §Cadence from the start**, so `#5` (cadence) builds the
  ack-pacing on top of this damage state without a breaking API change — the
  reason this is settled before implementation (CLAUDE.md: the bones correct
  from day one).
...
- Damage interacts with scrollback (#3): the consumer renders the *viewport*, so
  ...
  the screen scrolls — viewport-vs-screen damage mapping is refined alongside #5.
