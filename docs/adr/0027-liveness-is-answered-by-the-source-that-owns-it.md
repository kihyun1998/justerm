...
**Amended 2026-08-20 (#787), conformance map only.** `add_grid` had no row at all, though it is a
mid-life entry point that performs GL work. D3 derives its predicate; the row records why the
resolution is a stated cost rather than a guard, which is the question the map is for. No decision
changed — this is the record catching up with a call site #773 gave GL work to.
...
| `add_grid` (#787) | **none** | **D3 — resolved with a stated cost, not a defect.** It is a mid-life entry point that performs GL work, so D3 names its predicate; what it may *do* with the answer is what makes the row a ✓. Refusing is ruled out by this crate's contract (the constructor is the one entry point that refuses, and only because it has nothing to defer into). Deferring is not available either, and that is the non-obvious half: the five deferring setters defer the **cell** along with the atlas, while `add_grid` cannot — a consumer reads `cellWidth` back the moment it returns, and the cell is derived on the CPU by an ink-scan that a dead context does not obstruct. So it proceeds and pays **one thrown-away bake** when the configuration is new: measured `bakes()` +1 in the loss window, and `restore` bakes that configuration again. Bounded by two facts, both now asserted in `demo/context-loss-grids.html` — `render` cannot reach `Draw` while `gpu_work_must_wait()` holds (`action` and `must_defer` are complements over the same three inputs), and a grid born on a configuration is that configuration's own key-matching holder, so the restore always re-bakes it. **The second only became true at #788**, which corrected the bake set from "who holds this now" to "who will hold it after the reconcile" |
