...
- **Tab stops are explicit per-column state**, not a modulo: HTS sets, TBC clears, default every
  ...
  where it started. `CHT` (#898) is `HT` counted — the forward walk repeated, stopping at the first
  repeat that does not move — so it adds no third walk. One thing they deliberately do **not** mirror is the deferred wrap — see
...
- **The scroll region redefines what "scroll" means.** DECSTBM changes which rows `IND` / `RI` /
  ...
  that includes the relative moves: `CUU` / `CUD` stop at a margin from its inner side, and `CNL` /
  `CPL` are those two plus a carriage return.
...
- `justerm-core/src/term.rs` — the `Perform` implementation and every verb: `print`, `execute`,
  `csi_dispatch`, `esc_dispatch`, `osc_dispatch`, `unhook`, `put_tab` / `put_back_tab` / `put_forward_tabs`, and the
