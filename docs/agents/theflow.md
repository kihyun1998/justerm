...
| A box with **no area is refused**, not floored — `proposeDimensions` / `gridForBox` answer `undefined` for a `0x0` container, the way they already do for a `NaN` one | **all three floor it**: xterm.js `Math.max(Constants.MINIMUM_COLS, ...)` with no zero branch (`addons/addon-fit/src/FitAddon.ts` @ `699f553`), alacritty `cmp::max(columns as usize, MIN_COLUMNS)` (`alacritty/src/display/mod.rs:249` @ `852e971`), ghostty `@max(1, calc_cols)` (`src/renderer/size.zig:260` @ `e6e26e1`) | #810, on the **consumer-facing API shape** row — our own coherence, since `proposeDimensions` already declares that a box meaning *"not measured"* is exactly when not to shrink, and a `display: none` box means that while being finite. **Why the convergence does not bind us**: alacritty and ghostty own an OS window, which cannot be `display: none`, so a zero box is a degenerate state there and flooring is a sane recovery. xterm.js is in the same world and still floors — but its `FitAddon` registers **no listeners**, so a hidden terminal is simply never fitted, while `observeResize` drives ours from a `ResizeObserver` that `display: none` fires. We automated what they left manual and therefore receive an input they do not. Severity is what settled it rather than tidiness: the floor proposes `2x1`, and on the **alt screen** a resize drops rows outright (`docs/map/territory/reflow.md`, #567), so the wrong answer is irreversible |
