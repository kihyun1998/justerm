...
| **0 of 3 references reflow the alt screen.** ghostty: *"Alternate screen, if it exists, doesn't reflow"*; alacritty passes `reflow` as `!is_alt` / `is_alt` so whichever grid is the alt one gets `false`; xterm.js gates reflow on `_hasScrollback` and builds the alt buffer with `hasScrollback = false`. **justerm does reflow it, and no ADR or `architecture.md` line records why** (the only artefact is a code comment that takes it as given). This divergence is the sole reason justerm has an "alt column resize must not destroy content" constraint at all, and it is what makes ghostty's own cursor-widening model unportable here | all three | ghostty `terminal/Terminal.zig` `resize` (the comment above the alt branch), alacritty `alacritty_terminal/src/term/mod.rs:677-678`, xterm.js `common/buffer/Buffer.ts:315-320` + `common/buffer/BufferSet.ts:47` |
