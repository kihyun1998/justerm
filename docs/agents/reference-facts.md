...
| A pop on an **empty** stack does nothing — 3-for-3 | xterm, xterm.js, alacritty | `misc.c:7994` (`result = False`); `InputHandler.ts:2966`; `mod.rs:2252` |
...
| The icon name is **never surfaced** to the consumer | xterm.js | `src/common/InputHandler.ts:3049` (`setIconName` fires nothing) |
