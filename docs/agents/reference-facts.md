...
| ⚠ **That bail is TWO questions, and the second is why xterm can afford to fail both at once (#921, verified 2026-09-16).** The guard is `!isCursorInViewport || isComposing`, and `isCursorInViewport` is **geometric** — `ybase + y - ydisp` inside `rows` — i.e. *the cursor is scrolled out of the window*, with no DECTCEM term in it; the voluntary writer checks the very same thing. **Neither bail can accumulate**, because xterm retains no cursor cell at all: both sites position from the live `buffer.x`/`.y` on every call, so a skipped write is exactly one skipped write. justerm-web's equivalent guard sat in front of a *retained* cell and so turned the same bail into a freeze lasting the whole scrolled-up excursion — and its one bit (`cursor.visible && display_offset == 0`) also folds in the DECTCEM question xterm keeps out of it | xterm.js | bail `browser/CoreBrowserTerminal.ts:338`; definition `common/buffer/Buffer.ts:109-113`; the voluntary writer's copy `browser/input/CompositionHelper.ts:250` @ `699f553` |
...
| ⚠ **Negative result: neither grid-drawing reference LATCHES a preedit origin, so neither can arbitrate what it should be latched from (#911, verified 2026-09-16).** ghostty recomputes `preedit_range` from `state.cursor.viewport` inside `rebuildCells`, every rebuild; alacritty takes a fresh `point` argument into `draw_ime_preview` on every `draw`. Both therefore read the **live** cursor, so an echo arriving a frame later corrects them with no rule and no state — the defect #911 names cannot occur there. Latching is justerm's own, forced by #637 (the OS re-reads the anchor, so the run may not follow output), and it is what turns "eventually right" into "wrong for the composition's whole life". **Read the reasons, not the count**: the shape has no vote here | ghostty · alacritty | ghostty `src/renderer/generic.zig:2369-2381` @ `e6e26e1`; alacritty `alacritty/src/display/mod.rs:1129` and its call site `:960` @ `852e971` |
| **…and neither does the one reference that shares this layer's shape (#911, verified 2026-09-16).** xterm.js is the structural sibling — a browser widget, a hidden textarea, a commit whose echo arrives asynchronously — so it is the one to ask about a preedit origin, and it does not latch: `updateCompositionElements` positions the composition view and the textarea from `bufferService.buffer.x`/`.y`, the **live** cursor, on every call, and then re-runs itself once through `setTimeout(0)` because *"the IME events across browsers are not consistently triggered"*. So it carries #911's symptom too and heals it on the next render. **3 of 3 re-read; the latch is justerm's own**, forced by #637, and #911 is its price | xterm.js | `src/browser/input/CompositionHelper.ts:245` (the method), `:250-252` (the live read), `:280-282` (the re-run) @ `699f553` |
...
| **Pointer-down is `preventDefault()` then focus, in the reference too.** justerm-web's `onDown` focuses the hidden textarea and relied on the *consumer* cancelling the default action until #902, which made it cancel every press it reports or hands to `TerminalOptions.selection`; xterm does both itself in one handler. Family + reference agree on the pairing, so this is not a divergence — but justerm-web's half of it was undocumented until #649, and `TerminalOptions.element` claimed the opposite (*"the widget makes it focusable"*, with no `tabIndex` anywhere) | xterm.js | `src/browser/services/MouseService.ts:226` (after `preventDefault()` at `:225`) |
...
| **A reset does not clear the stack — 1–2 against justerm's own behaviour.** xterm.js's `reset()` touches only attribute data; xterm's only bulk free is widget teardown, not RIS. Only alacritty clears. ⚠ **This row is the *stack* and does not generalise to the retained string**, which is 2–2 — ghostty clears the title but holds no stack, so it is absent here and present there (§ the sibling question, #835) | xterm.js, xterm, alacritty | `src/common/InputHandler.ts:3428`; `charproc.c:12135` (inside `VTDestroy`); `mod.rs:1845` |
