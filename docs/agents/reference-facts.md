...
### The IME anchor: nobody caches it, and xterm shares our staleness (#631, verified 2026-07-30)
...
| ⚠ **UNADJUDICATED — does the OS IME re-read the anchor mid-composition?** xterm cannot arbitrate: `_syncTextArea` bails while composing *and* its comment says the position is "locked until hidden", yet `CompositionHelper.updateCompositionElements` rewrites `_textarea.style.left/top` on **every render while composing**. Both cannot be load-bearing. Answerable only by a real IME, not by reading source | xterm.js | bail `browser/CoreBrowserTerminal.ts:338`; rewrite `browser/input/CompositionHelper.ts:245` (`updateCompositionElements`), the textarea writes at `:273-274`, driven by `browser/CoreBrowserTerminal.ts:430` |
