...
| `gate` | 1 | 4 groups. **The count is not stated here** — `gates.mjs --list` is authoritative and asserts its own list against `.github/workflows/test.yml` |
...
**An in-repo fact is cited by *symbol*, never by line.** `file:line` is sound for the trees above
because the **SHA pins the line**; this repository has no such pin and moves every commit, so the
same form here is a citation that goes false in silence — `cite.mjs --pins` checks the pinned side
and nothing checks this one. Name the function, the guard or the field and let `rg` find it. Measured
2026-09-03: three in-repo line citations existed across all of `docs/agents/`, one of them in this
file, and it had been wrong since `term.rs` was refactored — found by a lens walking past it, not by
any check.

**And a count that a command can produce is not written down here.** The divergence table already
carries that rule (it went stale at 10 vs 12 in three days); the same failure reached the gate list
and the tree-rule table, which stated **22 commands** while the runner held 23 and **78 test files**
while the tree held 82. The load-bearing claim in both is a *property* — the list matches CI, the
sort has no overlap — and a property does not drift.
...
| core: inline `mod tests` vs the files in `tests/` | module-internal unit / through the public API | **overlap 0** — no module is tested from both sides |
| renderer: inline everywhere vs **no** `tests/` directory | pure module / `webgl.rs` + `rasterizer.rs`, wasm32-only and 0-compiling on host, so their proof is `e2e/` | **every module but those two** |
...
### `gate` — 4 groups, each command run bare
...
| **CBT (`CSI Ps Z`) clears the deferred wrap**, so on a full row the next character lands where the back-tab put it | **all four leave it armed**, so the same stream puts that character on the *following* row and the back-tab is in effect discarded — alacritty writes no `input_needs_wrap` though its own CUB does (`alacritty_terminal/src/term/mod.rs:1253`), ghostty's `horizontalTabBack` routes through the *screen*-level `cursorLeft` where the *terminal*-level one clears it (`src/terminal/Terminal.zig:1768`), xterm's `TabToPrevStop` has no `ResetWrap` — its one occurrence in `tabs.c` is in the **forward** walk and is `curses`-gated (`tabs.c:113`, default `False` at `charproc.c:434`) — and xterm.js has no flag, returning early on `x >= cols` (`src/common/InputHandler.ts:1142`) | #826. **The tie-breaker cannot award this**, which is why the head count does not decide it: the VT row gives the spec top authority, but CBT's normative home is ECMA-48 and **no pinned tree carries it** — `ctlseqs.txt` is xterm's documentation *of xterm*. The argument that needs no such document: the deferred wrap is an **implementation device** for a cursor parked past the last column, not an ECMA-48 concept, so no version of that spec could rule on it. What decides it is this engine's own coherence — every horizontal-positioning verb here clears the flag — plus the fact that leaving it armed reproduces #826's own symptom from the other side. Rows in [`reference-facts.md`](reference-facts.md) |
