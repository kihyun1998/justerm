...
impl DecodedFrame {
    ...
    /// Decoration markers visible in this viewport, `MARKER_STRIDE`
...
/// the wire). A consumer can read it at load time to assert the WASM decoder and
...
/// bits *inside* the value and no enum can answer "which of five" about a set. A value space's
/// names live at module scope, and the shape follows from the value being a set rather than a
/// choice.
#[wasm_bindgen]
...
/// in prose.
///
/// The discriminants are the wire's, and adding one moves `WIRE_VERSION` — a louder
...
/// Decode a justerm wire buffer into a `DecodedFrame`.
///
/// On a malformed buffer this throws a JS `Error` whose `message` is the
/// `DecodeError` variant name — the validation a hand-written TS
