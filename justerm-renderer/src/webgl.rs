...
impl JustermRenderer {
    ...
    /// **and the VAO that points at it** (no selector, not shareable, cheap to create;
    ...
    /// reintroduce exactly the re-attach cost a shared surface exists to remove.
    ///
    ...
    /// Number of atlas bakes run so far (diagnostic) — every configuration built from nothing,
    ...
    /// Consume a decoded **damage** frame directly: scatter its span-ordered
    ...
    /// : a composition is browser-owned, reaches no frame and no wire, so the consumer is
    ...
    /// `setCursor` puts the caret on the composition's end, while DECTCEM still decides whether it
    /// is drawn at all.
    ...
    /// the palette *values* are the consumer's (theme-agnostic core), the *mechanism*
    ...
    /// reads `default_bg` fresh. Translucency needs no uniform re-push here: its trigger is the
    /// packer's per-cell `bg_default` provenance flag, which is palette-independent.
    ...
    /// frame, this renderer cannot self-refresh — the split mirrors the cursor's, and the consumer wires both.
    ...
    /// composites). Marks the buffer dirty; the next `render`
    /// re-packs.
    ...
    /// and both references draw it that way. It is not one because that is **the contract, not an
    ...
    ///
    /// **It is an echo, not an adopted value.** The drawing buffer belongs to the *surface* — N
    ...
    /// consumer's to compute from its own box, as xterm's `FitAddon` computes it.
    ...
    /// consumer injects this policy to make the terminal background see-through to the
    ...
    /// it is** . It used to contribute in full: an antialiased glyph
    ...
    /// `mix(u_bg_alpha, 1.0, cov)`, which was replaced — the `NaN` now reaches the colour
    ...
    /// default (xterm's default). Consumer policy: the mechanism (index remap at resolve)
    ...
    /// Consumer policy: the mechanism (the WCAG adjustment on the resolved RGB) is the
    ...
    /// cell. The mechanism is the renderer's — only it has the *resolved* per-cell RGB —
    ...
    /// see the paragraph below the next one, which is where that decision is stated. A no-op if the
    /// ratio is unchanged; on
    /// error every old atlas is left intact and `dpr` unadvanced, so the next notification retries
    /// (self-healing).
    ///
    /// **This is the "rebuild all of them" path, not the "re-key one" path.** One
    ...
    /// new size, baking one only if no grid already stands on it. Consumer policy:
    ...
    ///
    /// **It moves that grid only** — a font selector is per-grid,
    ...
    /// — the renderer stays font-agnostic; loading a webfont (`@font-face` / `FontFace`)
    ...
    /// Extra space between columns, in **CSS pixels** — the consumer's policy, applied
    ...
    /// — VSCode tears its WebGL renderer down and falls back to a DOM one. The callback
    ...
    ///
    /// This writes one tier: the grid's dimensions. Sizing the drawing buffer is `resizeSurface`,
    /// and the separation is load-bearing once several grids share a canvas — with two grids in two
    /// cell sizes there is no cell the *buffer* can be a multiple of.
    ///
    /// This is what `cols` / `rows` report back, and what the frames
    /// fed to this grid are expected to carry. At least one cell each way.
    ///
    /// **Nothing clamps it to the grid's rect, and that is a decision.** The guarantee that a
    ...
    /// `cssHeight`, exactly as it did when the buffer came from a grid (beamterm's
    /// `auto_resize_canvas_css = false` is the same split). Forget it
    ...
    /// rect placed on that canvas is in device px. One canvas, one space. It also keeps that
    ...
    /// Number of instance-buffer packs run so far (diagnostic). The consumer/proofs read the
